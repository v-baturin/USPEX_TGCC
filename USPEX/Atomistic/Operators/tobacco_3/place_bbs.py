...
def place_nodes(nvecs, charges, ORIENTATION_DEPENDENT_NODES,nodes_directory = 'nodes'):
	sbu_atoms_scaled_coordinates = []
	...
	for n in nvecs:
		...
		bbxvec = np.array(X_vecs(cif,nodes_directory,False))
		...
		bonds = bbbonds(cif, nodes_directory)
		...
		atoms_coordinates = np.c_[anf,aff_all]
		sbu_atoms_scaled_coordinates.append((name,atoms_coordinates))
		placed_nbb_coords_extend(laff_all)
		all_bonds_extend(abf)
		ind_seg = ind_seg + len(all_names)

	return placed_nbb_coords, all_bonds, sbu_atoms_scaled_coordinates

def place_edges(evecs, charges, nnodes,edges_direc = 'edges'):
	edges_atoms_coordinates = []
	...
	for e in evecs:
		...
		bbxvec  = np.array(X_vecs(cif,edges_direc,False))
		nbbxvec = np.array([ll*(v / np.linalg.norm(v)) for v in bbxvec])

		min_dist,rot,tran = superimpose(nbbxvec,evec)

		all_bb = bb2array(cif, edges_direc)
		all_coords = np.array([v[1] for v in all_bb])
		all_inds = np.array([v[0] for v in all_bb])
		chg, elem = bbcharges(cif, edges_direc)
		all_names = [o + re.sub('[A-Za-z]','',p) for o,p in zip(elem,all_inds)]
		...
		atoms_coordinates = np.c_[anf,aff_all]
		edges_atoms_coordinates.append((index,atoms_coordinates))
		laff_all = np.c_[anf, aff_all, chg, all_inds, [bbind] * len(anf)]
	...
	return placed_ebb_coords, all_bonds, edges_atoms_coordinates
