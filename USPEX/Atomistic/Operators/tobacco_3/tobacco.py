...
PRINT = PRINT
CONNECTION_SITE_BOND_LENGTH = CONNECTION_SITE_BOND_LENGTH
WRITE_CHECK_FILES = WRITE_CHECK_FILES
WRITE_CIF = WRITE_CIF
ALL_NODE_COMBINATIONS = ALL_NODE_COMBINATIONS
USER_SPECIFIED_NODE_ASSIGNMENT = USER_SPECIFIED_NODE_ASSIGNMENT
COMBINATORIAL_EDGE_ASSIGNMENT = COMBINATORIAL_EDGE_ASSIGNMENT
CHARGES = CHARGES
SCALING_ITERATIONS = SCALING_ITERATIONS
SYMMETRY_TOL = SYMMETRY_TOL
BOND_TOL = BOND_TOL
ORIENTATION_DEPENDENT_NODES = ORIENTATION_DEPENDENT_NODES
PLACE_EDGES_BETWEEN_CONNECTION_POINTS = PLACE_EDGES_BETWEEN_CONNECTION_POINTS
RECORD_CALLBACK = RECORD_CALLBACK
OUTPUT_SCALING_DATA = OUTPUT_SCALING_DATA
FIX_UC = FIX_UC
MIN_CELL_LENGTH = MIN_CELL_LENGTH
OPT_METHOD = OPT_METHOD
PRE_SCALE = PRE_SCALE
SINGLE_METAL_MOFS_ONLY = SINGLE_METAL_MOFS_ONLY
MOFS_ONLY = MOFS_ONLY
MERGE_CATENATED_NETS = MERGE_CATENATED_NETS
...
def run_template(template):
	...
	print('=========================================================================================================')
	print()
	tobacco_path = os.path.split(os.path.split(template)[0])[0]
	nodes_path = os.path.join(tobacco_path,'nodes')
	edges_path = os.path.join(tobacco_path,'edges')
	templates_path = os.path.join(tobacco_path,'templates')

	cat_count = 0
	for net in ct2g(os.path.basename(template),template):
		...
		node_cns = [(cncalc(node, nodes_path), node) for node in os.listdir(nodes_path)]
		...
		vas = vertex_assign(TG, TVT, node_cns, unit_cell, USER_SPECIFIED_NODE_ASSIGNMENT, SYMMETRY_TOL, ALL_NODE_COMBINATIONS,nodes_dir=nodes_path)
		...
		for va in vas:
			...
			for ea in eas:
				...
				ea_dict = assign_node_vecs2edges(TG, unit_cell, SYMMETRY_TOL, template,nodes_path = nodes_path)
				...
				scaled_coords = omega2coords(start, TG, sc_omega_plus, (sc_a,sc_b,sc_c,sc_alpha,sc_beta,sc_gamma), num_vertices, template, g, WRITE_CHECK_FILES,templates_path=templates_path)
				nvecs,evecs = scaled_node_and_edge_vectors(scaled_coords, sc_omega_plus, sc_unit_cell, ea_dict)
				placed_nodes, node_bonds, sbu_atoms_scaled_coords = place_nodes(nvecs, CHARGES, ORIENTATION_DEPENDENT_NODES,nodes_directory=nodes_path)
				placed_edges, edge_bonds,edges_atoms_coordinates = place_edges(evecs, CHARGES, len(placed_nodes),edges_direc=edges_path)
				...
				if WRITE_CIF:
					...
					return (sbu_atoms_scaled_coords,edges_atoms_coordinates,TG,scaled_params)
...
def run_tobacco_serial(templates, CHARGES=CHARGES):
	...
	if IGNORE_ALL_ERRORS:
		...
		for template in templates:
			template_data = run_template(template)
			if not template_data is None:
				sbu_atoms_coordinates, edges_atoms_coordinates, graph, scaled_cell_params = template_data
				return (sbu_atoms_coordinates, edges_atoms_coordinates, graph,scaled_cell_params)
