...
def vertex_assign(TG, TVT, node_cns, unit_cell, USNA, SYM_TOL, ALL_NODE_COMBINATIONS,nodes_dir = 'nodes'):
...
def assign_node_vecs2edges(TG, unit_cell, SYM_TOL, template_name,nodes_path = 'nodes'):
	...
	for n in TG.nodes(data=True):
		...
		bbxlabels = np.array([l[0] for l in X_vecs(cif, nodes_path, True)])
		nodlabels = np.array([l[0] for l in node_vecs(n[0], TG, unit_cell, True)])

		bbxvec = X_vecs(cif, nodes_path, False)
