...
def omega2coords(start, TG, sc_omega_plus, uc_params, num_vertices, template, g, CHECK,templates_path = 'templates'):
	template = os.path.split(template)[-1]
	tobacco_path = os.path.split(templates_path)[0]
	template_database_path = os.path.join(tobacco_path,'template_database')
	...
	try:
		...
		path = os.path.join(template_database_path, template)
		with open(path, 'r') as tcif:
