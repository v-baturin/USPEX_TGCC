...
def convertFromPymatgen(pmgStructure, pbc, structureType, cellType, elementType):
    """
    ...
    cell = cellType.initFromCellVectors(pbc, cellVectors)
    species = [elementType(specie.name) for specie in pmgStructure.species]
    coordinates = pmgStructure.cart_coords
    structure = structureType(species, coordinates, cell)
    return structure
...
def convertToPymatgen(structure):
    ...
    elementType = type(atomTypes[0])
    species = [el.short_name for el in atomTypes]
    pmgStructure = Structure(cellVectors, species, coordinates, coords_are_cartesian=True)
    return pmgStructure, (structureType, cellType, elementType)
...
def constructSurfaceSlab(structure, pbc, plane, slabThickness, **kwargs):
    ...
    pmgStructure, *types = convertToPymatgen(structure)
    ...
    slabStructure = convertFromPymatgen(slab, pbc, *types)
...
def constructGrainsSlabs(structure, pbc, sigma, plane, rotAxis, slabThickness, **kwargs):
    ...
    lowerGrainStructure = convertFromPymatgen(lowerGrain, pbc, **types)
    upperGrainStructure = convertFromPymatgen(upperGrain, pbc, **types)
