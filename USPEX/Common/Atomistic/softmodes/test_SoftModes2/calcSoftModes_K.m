function [freq, eigvector] = calcSoftModes_K(N_val, val, numIons, lat,...
    coords, atomType, goodBonds, kVector0)
...
small_bond = -0.37*log(goodBonds); % 0.05-0.1 seems reasonable
...
for bt = 1 : bond_types
    ...
    if it+1 < N_bonds
        if bonds_sorted(it+1,3) < small_bond(at_types(bonds_sorted(it+1,1)), at_types(bonds_sorted(it+1,2)))  %max(max(small_bond))
...
for bt = 1 : bond_types
    while (bonds_sorted(it+1,4) == bt) && (it < N_bonds)
        ...
        H = X_ab*exp(-2.7*f_ab);
        %OUT = [OUT; cos_x, cos_y, cos_z, H, bonds_sorted(it, 1:4)];
        if a == b
