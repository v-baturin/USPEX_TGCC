function [bonds, bond_group] = CalcBonds(lat, coor, numIons, atomType,...
    max_bond, same_bond)
...
for i = 1:N_atom
    ...
    tmp2 = [i*ones(N_total,1), tmp_ID, tmp_dist, zeros(N_total,1)];
    tmp2(To_Delete,:) = [];
    bonds = [bonds; tmp2];
        ...
        for j = 1:length(ID)
            if bonds(ID(j),4)==0
                if isequal( type(bonds(i,1:2)), type(bonds(ID(j),1:2)) )
                    ...
                    bonds_sorted = [bonds_sorted; bonds(ID(j),:)];
                end
            end
        end
    end
end
bond_group = [1:1:bond_type];
