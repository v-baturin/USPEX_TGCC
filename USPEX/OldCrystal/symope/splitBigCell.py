...
def splitBigCell(config, fixRndSeed, startLat, splitInto, numIons, nsym, sym_coef, debug=False):
    """
    ...
    if True:#ORG_STRUC['nsymN'][0] == 0:  # H. Stokes code to create a crystal with given symmetry
        ...
        for l1 in range(5):
            ...
            coord_splitter, lat1, errorS = symope_crystal(config, fixRndSeed, nsym, genAtoms, np.asarray([lattice1]), sym_coef)
