...
class SHELL:
    ...
    async def submit(self, command: str,
        ...
        if uri and uri.startswith('local://'):
            sock = uri[len('local://'):]
            if not os.path.exists(sock):
                logger.error(f'Flux-broker: socket {sock} is lost. Broker down!')
        with open(calcFolder/input, 'r') as fi,\
            ...
            rc, _, _ = await self.connector.execute('flux ping')
            if rc != 0:
                logger.error(f'Flux ping returned {rc}: broker down')
