...
# def MaxBonds(SYSTEM : AtomicStructure):
#     '''
#     This program is to:
...
#                     bond1[3] = bond_type
#                     distance = bond1[2] + Element(atomType[atom_type_seq[int(bond1[0])]]).covalent_radius\
#                                         + Element(atomType[atom_type_seq[int(bond1[1])]]).covalent_radius
#                     # bonds1.append((int(bond1[0]), int(bond1[1]), bond1[2], bond_type, *[int(x) for x in bond1[4:]]))
#                     bonds1.append(Bond(SYSTEM[int(bond1[0])], SYSTEM[int(bond1[1])], distance=distance,
#                                        type=bond_type, direction=[int(x) for x in bond1[4:]]))
#
#
