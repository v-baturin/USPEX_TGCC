__author__ = 'mrakitin'
__doc__ = "Class for group names"


class SpaceGroups:
    def __init__(self):
        self.spaceG = [
                'P1',       'P-1',      'P2',       'P2_1',       'A2',       'Pm',       'Pa',       'Am',       'Aa',         'P2/m',
    ...
    def return_group(self, group_number):
        """
        The function returns the group name for the specified number.

        :param group_number: space group number.
        :return group_name: name of the space group.
        """
        self.group_number = group_number
        self.group_name = None
        try:
            self.group_number = int(self.group_number)
            if 0 < self.group_number < len(self.spaceG) + 1:
                self.group_name = self.spaceG[self.group_number - 1]
        except:
            msg = 'Wrong parameter specified: ' + str(self.group_number)
            print(msg)
        return self.group_name


    def return_number(self, group_name):
        self.group_number = None
        self.group_name = str(group_name)
        if self.group_name in self.spaceG:
            self.group_number = self.spaceG.index(self.group_name) + 1
        return self.group_number


    def return_all(self):
        for i in range(len(self.spaceG)):
            print('%3i: %s' % (i+1, self.spaceG[i]))
...
if __name__ == "__main__":
    ...
    print(obj.spaceG)
    print(obj.return_number('C2/c'))
    obj.return_group(40)
    print(obj.group_number, obj.group_name)
